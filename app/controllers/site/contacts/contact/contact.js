...
angular.module('GRPApp')
    .controller('ContactCtrl', function ($scope, $q, $filter, grpService, contactService, $state, $stateParams, $mdDialog, $timeout, regex) {
        'use strict';
        $scope.regex = regex;


        if ($stateParams.contactid === 'new') {
            grpService.RelatedContacts.create($stateParams.siteid).then(function (contact) {
        ...
        $scope.selectedItemChange = function (contact) {
            if (contact) {
                ...
                if (contact.attributes.GlobalID && found.length > 0) {
                    $timeout(function () {
                        $state.go('main.site.contacts.contact', {contactid: $scope.contact.attributes.GlobalID});
                    }, 500);
                }
            }
        };
        ...
        $scope.deleteContact = function (ev) {
            $scope.contact.relationship.delete('Are you sure you want to remove this contact from this site?', ev, '^');
        };
